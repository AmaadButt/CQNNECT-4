...
board_width, board_height = 800, 700
display_width, display_height = 1920, 1080

x = (display_width - board_width) / 2
y = (display_height - board_height) / 2

#simulator = Aer.get_backend('statevetor_simulator')
...
class GameUI:
    ...
    def __init__(self, player):
        ...
        self._screen = pygame.display.set_mode((1280, 720)) # 800, 700
        self._board_img = pygame.image.load("./img/board.png")
        self._board_img_numbers = pygame.image.load("./img/board_numbers.png")
        self._bloch_sphere = pygame.image.load("./img/bloch_sphere.png")
    ...
    def draw_board(self, player=None, row=-1, column=-1, q=False):
        if player is not None:
            ...
            if q:
                ...
                text_rect = q_text.get_rect(
                    center=(self.OFFSET + self.CHIP_RADIUS + self.CHIP_OFFSET * column + self.CHIP_SIZE * column,
        ...
        self._screen.blit(self._bloch_sphere, (self.OFFSET + 700, self.OFFSET - 10))
...
class Game:
    ...
    def game_loop(self):
        ...
        while not done:
            ...
            if update_ui:
                qp = self.generate_qpiece()
                if qp:
                    self._gameUI.draw_board(self.get_current_player(), row, column, q=qp)
                    r = random.randint(1, 7 - column)
                    self._gameUI.draw_board(self.get_current_player(), row, column+r, q=qp)
                else:
